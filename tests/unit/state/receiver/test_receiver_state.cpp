...
static int                test_context  = 1234567;
...
static constexpr SacnReceiverCallbacks kTestCallbacks = {
    ...
    source_limit_exceeded, &test_context};
static constexpr SacnReceiverConfig kTestReceiverConfig = {
    kTestUniverse, kTestCallbacks, {1, kSacnDmxAddressCount}, kSacnReceiverInfiniteSources, 0u, kSacnIpV4AndIpV6};
...
static TerminationSet            test_term_set = {{0u, 0u}, {nullptr, nullptr, 0u, nullptr, nullptr, nullptr}, nullptr};
...
protected:
  ...
  void SetUp() override
  {
    ...
    context.num_socket_refs = test_netints.size();

    auto socket_refs = gsl::make_span(context.socket_refs, context.num_socket_refs);
    for (int i = 0; i < test_netints.size(); ++i)
    {
      socket_refs[i].socket.handle  = kTestSocket + i;
      socket_refs[i].socket.ip_type = test_netints[i].iface.ip_type;
#if SACN_RECEIVER_SOCKET_PER_NIC
      socket_refs[i].socket.ifindex = test_netints[i].iface.index;
#endif
    }
  }
  ...
  SacnReceiver* AddReceiver(const SacnReceiverConfig& config, ReceiverAddMode add_mode = ReceiverAddMode::kComplete)
  {
    ...
    next_socket_ = kTestSocket;
    EXPECT_EQ(add_sacn_receiver(next_receiver_handle_++, &config, &netint_config, nullptr, &receiver), kEtcPalErrOk);
    if (receiver)
    {
  ...
  static SacnSourceDetector* AddSourceDetector(const SacnSourceDetectorConfig& config = kTestSourceDetectorConfig)
  {
  ...
  SacnTrackedSource* AddTrackedSource(SacnReceiver* receiver)
  {
    ...
    EXPECT_EQ(add_sacn_tracked_source(receiver, &next_source_cid_, kTestName, &test_netints[0].iface, 0u, 0u, &source),
              kEtcPalErrOk);
...
protected:
  ...
  static void InitTestData(uint8_t                    start_code,
                           uint16_t                   universe,
                           const uint8_t*             data            = nullptr,
                           size_t                     data_len        = 0u,
                           uint8_t                    flags           = 0u,
                           const EtcPalUuid&          source_cid      = kTestCid,
                           uint8_t                    sequence_number = seq_num_,
                           const EtcPalMcastNetintId& netint          = test_netints[0].iface)
  {
    init_sacn_data_send_buf(test_data_.data(), start_code, &source_cid, kTestName, kTestPriority, universe, 0u,
                            kTestPreview);

    if (data)
      update_send_buf_data(test_data_.data(), data, static_cast<uint16_t>(data_len), kDisableForceSync);

    test_data_.at(SACN_OPTS_OFFSET) |= flags;
  ...
  void TestSourceLimitExceeded(int configured_max, int expected_max)
  {
    ...
    source_limit_exceeded_fake.custom_fake = [](sacn_receiver_t, uint16_t universe, void* context) {
      ...
      EXPECT_EQ(context, &test_context);
    };
    ...
    for (int i = 0; i < expected_max; ++i)
    {
      ...
      InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, last_cid);
      RunThreadCycle();
    }

    EXPECT_EQ(source_limit_exceeded_fake.call_count, 0u);

    InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u,
                 etcpal::Uuid::V4().get());
    RunThreadCycle();
    ...
    // Remove a source.
    InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), SACN_OPTVAL_TERMINATED,
                 last_cid);
...
TEST_F(TestReceiverState, RespectsMaxTrackedSourceLimit)
{
  for (int i = 0; i < kNumTestUniverses; ++i)
  {
         ...
         ++j)
    {
      ...
      EXPECT_EQ(add_sacn_tracked_source(receiver, &cid, "name", &test_netints[0].iface, 0u, 0u, &source), kEtcPalErrOk);
    }
  }
}
...
{
  EXPECT_EQ(get_expired_wait(), static_cast<uint32_t>(kSacnDefaultExpiredWaitMs));
}
...
#if SACN_RECEIVER_SOCKET_PER_NIC
  EXPECT_EQ(sacn_remove_receiver_socket_fake.call_count, test_netints.size());
#else
...
TEST_F(TestReceiverState, ClearTermSetsAndSourcesWorks)
{
  ...
  receiver->term_sets = &test_term_set;
...
#if SACN_RECEIVER_SOCKET_PER_NIC
  EXPECT_EQ(sacn_add_receiver_socket_fake.call_count, test_netints.size());
#else
...
TEST_F(TestReceiverState, AddReceiverSocketsWorks)
{
  sacn_add_receiver_socket_fake.custom_fake = [](sacn_thread_id_t thread_id, etcpal_iptype_t ip_type, uint16_t universe,
    ...
    {
      bool found = false;
      for (const auto& test_netint : test_netints)
      {
        if ((netints[i].index == test_netint.iface.index) && (netints[i].ip_type == test_netint.iface.ip_type))
        {
          found = true;
          break;
        }
...
#if SACN_RECEIVER_SOCKET_PER_NIC
  EXPECT_EQ(sacn_add_receiver_socket_fake.call_count, test_netints.size() / 2);
#else
...
#if SACN_RECEIVER_SOCKET_PER_NIC
  EXPECT_EQ(sacn_remove_receiver_socket_fake.call_count, test_netints.size() / 2);
#else
...
{
#if SACN_RECEIVER_SOCKET_PER_NIC
  static const size_t kSocketsPerUniverse = test_netints.size();
#else
  ...
  sacn_remove_receiver_socket_fake.custom_fake = [](sacn_thread_id_t, etcpal_socket_t* socket, uint16_t,
                                                    const EtcPalMcastNetintId*, size_t,
                                                    sacn_socket_cleanup_behavior_t cleanup_behavior) {
    auto universe = static_cast<uint16_t>(kTestUniverse +
                                          ((sacn_remove_receiver_socket_fake.call_count - 1u) / kSocketsPerUniverse));
...
#if SACN_RECEIVER_SOCKET_PER_NIC
  EXPECT_EQ(sacn_remove_receiver_socket_fake.call_count, test_netints.size() * kNumTestUniverses);
#else
...
{
  SacnReceiver* receiver = AddReceiver();
  ASSERT_EQ(receiver->netints.num_netints, test_netints.size());

  std::vector<SacnTrackedSource*> sources;
  for (const auto& netint : test_netints)
  {
    EtcPalUuid         cid    = etcpal::Uuid::V4().get();
    SacnTrackedSource* source = nullptr;
...
TEST_F(TestReceiverThread, UniverseDataWorks)
{
  universe_data_fake.custom_fake = [](sacn_receiver_t receiver_handle, const EtcPalSockAddr* source_addr,
    ...
    EXPECT_EQ(universe_data->start_code, kSacnStartcodeDmx);
  ...
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size());
...
{
  static sacn_remote_source_t first_handle = kSacnRemoteSourceInvalid;
  ...
  universe_data_fake.custom_fake = [](sacn_receiver_t, const EtcPalSockAddr*, const SacnRemoteSource* source_info,
    ...
    EXPECT_NE(first_handle, kSacnRemoteSourceInvalid);
  };

  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, cid1);
  RunThreadCycle();
  ...
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, cid2);
  RunThreadCycle();
  ...
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, cid3);
  RunThreadCycle();
...
TEST_F(TestReceiverThread, UniverseDataFiltersPreview)
{
  ...
  filter_preview_config.flags |= kSacnReceiverOptsFilterPreviewData;
  UpdateTestReceiverConfig(filter_preview_config);

  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), SACN_OPTVAL_PREVIEW);
  RunThreadCycle();
  EXPECT_EQ(universe_data_fake.call_count, 0u);
...
{
  auto test_netints_middle                        = test_netints.begin() + (static_cast<int>(test_netints.size()) / 2);
  std::vector<SacnMcastInterface> current_netints = {test_netints.begin(), test_netints_middle};
  std::vector<SacnMcastInterface> future_netints  = {test_netints_middle, test_netints.end()};
  ...
  for (const auto& netint : current_netints)
  {
    InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, etcpal::Uuid::V4().get(),
...
TEST_F(TestReceiverThread, UniverseDataFiltersSubsequentSourceNetints)
{
  ...
  for (size_t i = 0; i < kNumTestIterations; ++i)
  {
    for (size_t j = 0; j < kNumSources; ++j)
    {
      ...
      for (size_t k = 0; k < test_netints.size(); ++k)
        ordered_netints.push_back(test_netints[(j + k) % test_netints.size()]);
      ...
      for (size_t k = 0; k < kNumDataCallsPerSource; ++k)
      {
        for (const auto& netint : ordered_netints)
        {
          ...
          uint8_t start_code = kSacnStartcodePriority;
#else
          uint8_t start_code = kSacnStartcodeDmx;
#endif
...
TEST_F(TestReceiverThread, UniverseDataFiltersUnknownUniverses)
{
  ...
  {
    InitTestData(kSacnStartcodeDmx, unknown_universe, kTestBuffer.data(), kTestBuffer.size());
    RunThreadCycle();
    EXPECT_EQ(universe_data_fake.call_count, 0u);
  }
...
TEST_F(TestReceiverThread, PapNotifiesCorrectlyDuringSamplingPeriod)
{
  universe_data_fake.custom_fake = [](sacn_receiver_t, const EtcPalSockAddr*, const SacnRemoteSource*,
                                      const SacnRecvUniverseData* universe_data,
                                      void*) { EXPECT_EQ(universe_data->start_code, kSacnStartcodePriority); };

  InitTestData(kSacnStartcodePriority, kTestUniverse, kTestBuffer.data(), kTestBuffer.size());
  RunThreadCycle();
#if SACN_ETC_PRIORITY_EXTENSION
  EXPECT_EQ(universe_data_fake.call_count, 1u);
#else
  ...
  universe_data_fake.custom_fake = [](sacn_receiver_t, const EtcPalSockAddr*, const SacnRemoteSource*,
                                      const SacnRecvUniverseData* universe_data,
                                      void*) { EXPECT_EQ(universe_data->start_code, kSacnStartcodeDmx); };
...
TEST_F(TestReceiverThread, StatusListsTrackMultipleSources)
{
  ...
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, offline_cid_1);
  RunThreadCycle();
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), SACN_OPTVAL_TERMINATED,
               offline_cid_1);
  RunThreadCycle();
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, offline_cid_2);
  RunThreadCycle();
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), SACN_OPTVAL_TERMINATED,
               offline_cid_2);
  RunThreadCycle();

  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, unknown_cid_1);
  RunThreadCycle();
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, unknown_cid_2);
  etcpal_getms_fake.return_val += (kSacnPeriodicInterval + 1u);
  RunThreadCycle();

  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, online_cid_1);
  RunThreadCycle();
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, online_cid_2);
  RunThreadCycle();
  ...
  static auto list_includes_cids = [](const EtcPalUuid& list_cid_1, const EtcPalUuid& list_cid_2,
    ...
    if ((ETCPAL_UUID_CMP(&list_cid_1, &actual_cid_2) == 0) && (ETCPAL_UUID_CMP(&list_cid_2, &actual_cid_1) == 0))
      return true;
...
TEST_F(TestReceiverThread, SourcesLostWorks)
{
  ...
  static std::array<EtcPalUuid, kNumLostSources> lost_source_cids;

  for (size_t i = 0u; i < kNumLostSources; ++i)
  {
    EXPECT_EQ(etcpal_rbtree_size(&test_receiver_->sources), i);
    lost_source_cids.at(i) = etcpal::Uuid::V4().get();
    InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, lost_source_cids.at(i));
    RunThreadCycle();
  ...
  get_expired_sources_fake.custom_fake = [](TerminationSet**, SourcesLostNotification* sources_lost) {
    for (size_t i = 0u; i < kNumLostSources; ++i)
      add_lost_source(sources_lost, GetHandle(lost_source_cids.at(i)), &lost_source_cids.at(i), kTestName,
                      (i % 2) == 0);
  };
  ...
  sources_lost_fake.custom_fake = [](sacn_receiver_t handle, uint16_t universe, const SacnLostSource* lost_sources,
    ...
    for (size_t i = 0u; i < kNumLostSources; ++i)
    {
      EXPECT_EQ(ETCPAL_UUID_CMP(&lost_sources[i].cid, &lost_source_cids.at(i)), 0);
...
{
  auto test_netints_middle                        = test_netints.begin() + (static_cast<int>(test_netints.size()) / 2);
  std::vector<SacnMcastInterface> initial_netints = {test_netints.begin(), test_netints_middle};
  std::vector<SacnMcastInterface> future_netints  = {test_netints_middle, test_netints.end()};
  ...
  for (auto* sp_netint = reinterpret_cast<SacnSamplingPeriodNetint*>(
...
{
  int configured_max = kSacnReceiverInfiniteSources;
  int expected_max   = SACN_RECEIVER_MAX_SOURCES_PER_UNIVERSE;
  TestSourceLimitExceeded(configured_max, expected_max);
}
...
{
  EXPECT_EQ(universe_data_fake.call_count, 0u);
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, kTestCid, 10u);
  RunThreadCycle();
  EXPECT_EQ(universe_data_fake.call_count, 1u);
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, kTestCid, 11u);
  RunThreadCycle();
  EXPECT_EQ(universe_data_fake.call_count, 2u);
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, kTestCid, 21u);
  RunThreadCycle();
  EXPECT_EQ(universe_data_fake.call_count, 3u);
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, kTestCid, 21u);
  RunThreadCycle();
  EXPECT_EQ(universe_data_fake.call_count, 3u);
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, kTestCid, 20u);
  RunThreadCycle();
  EXPECT_EQ(universe_data_fake.call_count, 3u);
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, kTestCid, 2u);
  RunThreadCycle();
  EXPECT_EQ(universe_data_fake.call_count, 3u);
  InitTestData(kSacnStartcodeDmx, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(), 0u, kTestCid, 1u);
  RunThreadCycle();
