...
static const std::vector<SacnSourceUniverseNetintList> kTestNetintLists = {
    {kTestHandle, kTestLargerUniverse2, test_netints.data(), test_netints.size(), false},
    {kTestHandle, kTestSmallerUniverse, test_netints.data(), test_netints.size(), false},
    {kTestHandle2, kTestLargerUniverse2, test_netints.data(), test_netints.size(), false},
    {kTestHandle2, kTestSmallerUniverse, test_netints.data(), test_netints.size(), false}};
...
static const std::vector<SacnSourceUniverseNetintList> kTestInvalidNetintLists1 = {
    {kTestHandle, kTestUniverse, test_netints.data(), test_netints.size(), false},
    {kTestHandle, kTestUniverse2, test_netints.data(), test_netints.size(), false}};
static const std::vector<SacnSourceUniverseNetintList> kTestInvalidNetintLists2 = {
    {kTestHandle, kTestUniverse, test_netints.data(), test_netints.size(), false},
    {kTestHandle2, kTestUniverse2, test_netints.data(), test_netints.size(), false}};
static const std::vector<SacnSourceUniverseNetintList> kTestInvalidNetintLists3 = {
    {kTestHandle, kTestUniverse, test_netints.data(), test_netints.size(), false},
    {kTestHandle, kTestUniverse2, test_netints.data(), test_netints.size(), false},
    {kTestHandle2, kTestUniverse, test_netints.data(), test_netints.size(), false},
    {kTestHandle2, kTestUniverse2, test_netints.data(), test_netints.size(), false},
    {kTestHandle2, kTestUniverse3, test_netints.data(), test_netints.size(), false}};
...
protected:
  void SetUp() override
  {
    ...
    sacn_initialize_source_netints_fake.custom_fake = [](SacnInternalNetintArray* source_netints,
      ...
      source_netints->netints_capacity = test_netints.size();
#endif
      source_netints->num_netints = test_netints.size();
      ...
      if (app_netint_config)
      {
        ...
        {
          EXPECT_EQ(app_netint_config->netints[i].iface.index, test_netints[i].iface.index);
          EXPECT_EQ(app_netint_config->netints[i].iface.ip_type, test_netints[i].iface.ip_type);
          EXPECT_EQ(app_netint_config->netints[i].status, test_netints[i].status);
  ...
  static void SetUpSource(sacn_source_t source_handle)
  {
  ...
  static void SetUpSourceAndUniverse(sacn_source_t source_handle, uint16_t universe_id)
  {
  ...
  static void SetUpSourcesAndUniverses(const std::vector<SacnSourceUniverseNetintList>& netint_lists,
...
TEST_F(TestSource, SourceConfigInitWorks)
{
  ...
  EXPECT_EQ(config.universe_count_max, (size_t)kSacnSourceInfiniteUniverses);
...
TEST_F(TestSource, SourceDestroyHandlesInvalidHandle)
{
  ...
  VERIFY_NO_LOCKING(sacn_source_destroy(kSacnSourceInvalid));
  EXPECT_EQ(set_source_terminating_fake.call_count, 0u);
  VERIFY_LOCKING(sacn_source_destroy(kTestHandle));
  EXPECT_EQ(set_source_terminating_fake.call_count, 1u);
}
...
TEST_F(TestSource, SourceChangeNameErrInvalidWorks)
{
  ...
  VERIFY_NO_LOCKING_AND_RETURN_VALUE(sacn_source_change_name(kSacnSourceInvalid, kTestLocalName2), kEtcPalErrInvalid);
...
TEST_F(TestSource, SourceAddUniverseErrInvalidWorks)
{
  ...
  VERIFY_NO_LOCKING_AND_RETURN_VALUE(sacn_source_add_universe(kSacnSourceInvalid, &valid_config, &netint_config),
                                     kEtcPalErrInvalid);
...
TEST_F(TestSource, SourceRemoveUniverseWorks)
{
  ...
  set_universe_terminating_fake.custom_fake = [](SacnSourceUniverse* universe, sacn_set_terminating_behavior_t) {
...
TEST_F(TestSource, SourceAddUnicastDestinationErrInvalidWorks)
{
  SetUpSourceAndUniverse(kTestHandle, kTestUniverse);
  VERIFY_NO_LOCKING_AND_RETURN_VALUE(
      sacn_source_add_unicast_destination(kSacnSourceInvalid, kTestUniverse, kTestRemoteAddrs.data()),
      kEtcPalErrInvalid);
...
TEST_F(TestSource, SourceChangePriorityErrInvalidWorks)
{
  ...
  VERIFY_NO_LOCKING_AND_RETURN_VALUE(sacn_source_change_priority(kSacnSourceInvalid, kTestUniverse, kTestPriority),
                                     kEtcPalErrInvalid);
...
TEST_F(TestSource, SourceChangePreviewFlagErrInvalidWorks)
{
  ...
  VERIFY_NO_LOCKING_AND_RETURN_VALUE(sacn_source_change_preview_flag(kSacnSourceInvalid, kTestUniverse, true),
                                     kEtcPalErrInvalid);
...
TEST_F(TestSource, SourceSendNowErrInvalidWorks)
{
  ...
  VERIFY_NO_LOCKING_AND_RETURN_VALUE(
      sacn_source_send_now(kSacnSourceInvalid, kTestUniverse, kTestStartCode, kTestBuffer.data(), kTestBuffer.size()),
      kEtcPalErrInvalid);
  VERIFY_NO_LOCKING_AND_RETURN_VALUE(
  ...
  VERIFY_NO_LOCKING_AND_RETURN_VALUE(
      ...
      kEtcPalErrInvalid);
  VERIFY_NO_LOCKING_AND_RETURN_VALUE(
      sacn_source_send_now(kTestHandle, kTestUniverse, kTestStartCode, kTestBuffer.data(), (kSacnDmxAddressCount + 1u)),
      kEtcPalErrInvalid);
  VERIFY_NO_LOCKING_AND_RETURN_VALUE(
...
TEST_F(TestSource, SourceUpdateValuesWorks)
{
  ...
  update_levels_and_or_pap_fake.custom_fake =
      [](SacnSource* source, SacnSourceUniverse* universe, const uint8_t* new_levels, size_t new_levels_size,
         const uint8_t* new_priorities, size_t new_priorities_size, sacn_force_sync_behavior_t force_sync) {
        EXPECT_EQ(source->handle, kTestHandle);
        EXPECT_EQ(universe->universe_id, kTestUniverse);
...
TEST_F(TestSource, SourceUpdateValuesHandlesInvalid)
{
  ...
  VERIFY_NO_LOCKING(
      sacn_source_update_levels(kTestHandle, kTestUniverse, kTestBuffer.data(), (kSacnDmxAddressCount + 1u)));
...
TEST_F(TestSource, SourceUpdateValuesAndPapHandlesInvalid)
{
  ...
  VERIFY_NO_LOCKING(sacn_source_update_levels_and_pap(kTestHandle, kTestUniverse, kTestBuffer.data(),
                                                      (kSacnDmxAddressCount + 1u), kTestBuffer2.data(),
                                                      kTestBuffer2.size()));
  EXPECT_EQ(update_levels_and_or_pap_fake.call_count, 0u);
  VERIFY_NO_LOCKING(sacn_source_update_levels_and_pap(kTestHandle, kTestUniverse, kTestBuffer.data(),
                                                      kTestBuffer.size(), kTestBuffer2.data(),
                                                      (kSacnDmxAddressCount + 1u)));
  EXPECT_EQ(update_levels_and_or_pap_fake.call_count, 0u);
  VERIFY_LOCKING(sacn_source_update_levels_and_pap(kTestHandle, kTestUniverse, kTestBuffer.data(), kTestBuffer.size(),
                                                   kTestBuffer2.data(), kTestBuffer2.size()));
  EXPECT_EQ(update_levels_and_or_pap_fake.call_count, 1u);
...
{
  take_lock_and_process_sources_fake.custom_fake = [](sacn_process_sources_behavior_t behavior,
...
TEST_F(TestSource, SourceResetNetworkingWorks)
{
  ...
  sacn_sockets_reset_source_fake.custom_fake = [](const SacnNetintConfig* netint_config) {
    ...
    if (netint_config)
    {
      ...
      {
        EXPECT_EQ(netint_config->netints[i].iface.index, test_netints[i].iface.index);
        EXPECT_EQ(netint_config->netints[i].iface.ip_type, test_netints[i].iface.ip_type);
        EXPECT_EQ(netint_config->netints[i].status, test_netints[i].status);
      }
    }
