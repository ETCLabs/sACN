"""Script to create a new versioned build of sACN.

Updates the sacn/version.h file
Prompts the user to review before committing and tagging

This script is meant to be run by an sACN developer on a development workstation. It assumes that
...
COMMIT_MSG_TEMPLATE = 'Update version files for sACN build {}'
TAG_MSG_TEMPLATE = 'sACN version {}'
RELEASE_TAG_MSG_TEMPLATE = 'sACN release version {}'
...
def resolve_import_versions(repo_root, version):
    ...
    import_file_contents = import_file_contents.replace('@SACN_VERSION_STRING@', version_str)
...
def update_version_files(repo_root, version):
    ...
    for in_file, out_file in zip(in_file_handles, out_file_handles):
        for line in in_file.readlines():
            line = line.replace('@SACN_VERSION_MAJOR@', str(version[0]))
            line = line.replace('@SACN_VERSION_MINOR@', str(version[1]))
            line = line.replace('@SACN_VERSION_PATCH@', str(version[2]))
            line = line.replace('@SACN_VERSION_BUILD@', str(version[3]))
            line = line.replace('@SACN_VERSION_STRING@', version_str)
            line = line.replace('@SACN_VERSION_DATESTR@', today.strftime('%d.%b.%Y'))
            line = line.replace('@SACN_VERSION_COPYRIGHT@', 'Copyright ' + str(today.year) + ' ETC Inc.')
            out_file.write(line)
        in_file.close()
        out_file.close()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Create a new versioned build of sACN')
    parser.add_argument('new_version', help='New version number (format M.m.p.b)')
    parser.add_argument('-r', '--release', action='store_true', help='Tag a release build of sACN.')
