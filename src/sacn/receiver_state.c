...
static void sacn_process_thread(void* arg);

static etcpal_error_t add_sockets(sacn_thread_id_t           thread_id,
...
static void process_null_start_code(const SacnReceiver*        receiver,
...
#endif
static void process_new_source_data(SacnReceiver*                    receiver,
...
static void process_receiver_sources(sacn_thread_id_t         thread_id,
...
void sacn_receiver_state_deinit(void)
{
  ...
  etcpal_thread_t* threads_handles_to_deinit[SACN_RECEIVER_MAX_THREADS * 2] = {NULL};  // 2 threads per context
  ...
  if (sacn_receiver_lock())
  {
    for (unsigned int i = 0; i < sacn_mem_get_num_threads(); ++i)
    {
      ...
      {
        etcpal_signal_post(&thread_context->process_thread_deinit_signal);
        etcpal_signal_post(&thread_context->recv_thread_deinit_signal);
        threads_ids_to_deinit[num_threads_to_deinit]               = thread_context->thread_id;
        threads_handles_to_deinit[num_threads_to_deinit * 2]       = &thread_context->process_thread_handle;
        threads_handles_to_deinit[(num_threads_to_deinit * 2) + 1] = &thread_context->recv_thread_handle;
  ...
  for (int i = 0; i < (num_threads_to_deinit * 2); ++i)
...
void tick_process_thread(SacnRecvThreadContext* context)
{
  ...
  if (read_event != kSacnReadEventTimedOut)
  {
    ...
    SACN_ASSERT_VERIFY(
        sacn_sem_post((read_event == kSacnReadEventNetwork) ? &context->network_sem : &context->hook_sem));
  }
...
/*
 * Start a new thread (technically 2 threads) to process receiver state. These threads are associated with a specific
 * SacnRecvThreadContext instance.
 *
 * [in,out] recv_thread_context Thread context corresponding to the threads to start.
...
/*
 * The network receive thread function. Manages sockets and polls/receives from the network, forwarding events to the
 * process thread.
 */
void sacn_receive_thread(void* arg)
{
  ...
  while (!etcpal_signal_try_wait(&context->recv_thread_deinit_signal))
...
/*
 * The process thread function. Processes data from the receive hook or network, as well as periodic timeouts for sACN
 * receivers.
 */
...
etcpal_error_t add_sockets(sacn_thread_id_t           thread_id,
                           etcpal_iptype_t            ip_type,
                           uint16_t                   universe,
                           const EtcPalMcastNetintId* netints,
                           size_t                     num_netints,
 ...
 * [in] read_data->data Incoming data buffer.
 * [in] read_data->data_len Size of data buffer.
 * [in] read_data->from_addr Network address from which the data was received.
 * [in] read_data->netint ID of network interface on which the data was received.
 */
void handle_incoming(SacnRecvThreadContext* context, const SacnReadData* read_data)
{
  if (!SACN_ASSERT_VERIFY(context) || !SACN_ASSERT_VERIFY(read_data) || !SACN_ASSERT_VERIFY(read_data->data))
    return;

  AcnUdpPreamble preamble;
  if (!acn_parse_udp_preamble(read_data->data, read_data->data_len, &preamble))
    return;
  ...
  while (acn_parse_root_layer_pdu(preamble.rlp_block, preamble.rlp_block_len, &rlp, &lpdu))
  {
    if (rlp.vector == ACN_VECTOR_ROOT_E131_DATA)
      handle_sacn_data_packet(context->thread_id, &rlp, read_data);
    else if (rlp.vector == ACN_VECTOR_ROOT_E131_EXTENDED)
      handle_sacn_extended_packet(context, rlp.pdata, rlp.data_len, &rlp.sender_cid, &read_data->from_addr);
  }
}
 ...
 * [in] read_data->netint ID of network interface on which the data was received.
 */
void handle_sacn_data_packet(sacn_thread_id_t thread_id, const AcnRootLayerPdu* rlp, const SacnReadData* read_data)
{
  if (!SACN_ASSERT_VERIFY(thread_id != kSacnThreadIdInvalid) || !SACN_ASSERT_VERIFY(rlp) ||
      !SACN_ASSERT_VERIFY(rlp->pdata) || !SACN_ASSERT_VERIFY(read_data))
  {
    return;
  }
  ...
  if (receiver_cb_lock())
  {
    ...
    if (sacn_receiver_lock())
    {
      ...
      if (src)
      {
        ...
        if ((src->netint.ip_type != read_data->netint.ip_type) || (src->netint.index != read_data->netint.index))
        {
      ...
      {
        process_new_source_data(receiver, &universe_data->source_info, &read_data->netint,
    ...
    deliver_receive_callbacks(&read_data->from_addr, &universe_data->source_info,
 ...
 */
void handle_sacn_extended_packet(SacnRecvThreadContext* context,
                                 const uint8_t*         data,
                                 size_t                 datalen,
                                 const EtcPalUuid*      sender_cid,
                                 const EtcPalSockAddr*  from_addr)
 ...
 */
void process_null_start_code(const SacnReceiver*        receiver,
                             SacnTrackedSource*         src,
                             SourcePapLostNotification* source_pap_lost,
                             bool*                      notify)
{
 ...
 * [out] notify Whether or not to forward the data to the user in a notification.
 */
void process_new_source_data(SacnReceiver*                    receiver,
                             const SacnRemoteSource*          source_info,
                             const EtcPalMcastNetintId*       netint,
                             const SacnRecvUniverseData*      universe_data,
                             SacnTrackedSource**              new_source,
...
void deliver_receive_callbacks(const EtcPalSockAddr*            from_addr,
                               const SacnRemoteSource*          source_info,
                               uint16_t                         universe_id,
                               SourceLimitExceededNotification* source_limit_exceeded,
                               SourcePapLostNotification*       source_pap_lost,
