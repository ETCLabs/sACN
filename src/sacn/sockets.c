...
static etcpal_error_t test_sacn_receiver_netint(unsigned int           index,
                                                ...
                                                SacnSocketsSysNetints* sys_netints);
static etcpal_error_t test_sacn_source_netint(unsigned int                index,
                                              etcpal_iptype_t             ip_type,
                                              const EtcPalIpAddr*         addr,
                                              const SacnSendSocketConfig* send_socket_config);
static etcpal_error_t init_unicast_send_sockets(const SacnSendSocketConfig* config);
static bool           add_sacn_sys_netint(const EtcPalMcastNetintId* netint_id,
...
static etcpal_error_t create_multicast_send_socket(const EtcPalMcastNetintId*  netint_id,
                                                   const SacnSendSocketConfig* config,
                                                   etcpal_socket_t*            socket);
static etcpal_error_t create_unicast_send_socket(etcpal_iptype_t             ip_type,
                                                 const SacnSendSocketConfig* config,
                                                 etcpal_socket_t*            socket);
#if SACN_FULL_OS_AVAILABLE_HINT
static void configure_sndbuf_size(etcpal_socket_t new_sock, const char* sock_desc);
#endif
static etcpal_error_t create_receive_socket(etcpal_iptype_t       ip_type,
                                            const EtcPalSockAddr* bind_addr,
                                            bool                  set_sockopts,
                                            ReceiveSocket*        socket);
static void           poll_add_socket(SacnRecvThreadContext* recv_thread_context, ReceiveSocket* socket);
#if SACN_RECEIVER_ENABLED || DOXYGEN
static etcpal_error_t queue_subscription(SacnRecvThreadContext*     recv_thread_context,
...
static etcpal_error_t send_unicast(const uint8_t*      send_buf,
 ...
 */
etcpal_error_t create_multicast_send_socket(const EtcPalMcastNetintId*  netint_id,
                                            const SacnSendSocketConfig* config,
                                            etcpal_socket_t*            socket)
{
  if (!SACN_ASSERT_VERIFY(netint_id) || !SACN_ASSERT_VERIFY(config) || !SACN_ASSERT_VERIFY(socket))
    return kEtcPalErrSys;
  ...
  if ((res == kEtcPalErrOk) && (config->multicast_sndtimeo_ms > 0))
  {
    res = etcpal_setsockopt(new_sock, ETCPAL_SOL_SOCKET, ETCPAL_SO_SNDTIMEO, &config->multicast_sndtimeo_ms,
                            sizeof config->multicast_sndtimeo_ms);

    if (res != kEtcPalErrOk)
    {
      SACN_LOG_ERR("Failed to set SO_SNDTIMEO to %d ms on %s multicast socket: '%s'", config->multicast_sndtimeo_ms,
                   sockopt_ip_level_str, etcpal_strerror(res));
    }
  }
 ...
 * Returns kEtcPalErrOk (success) or a relevant error code on failure.
 */
etcpal_error_t create_unicast_send_socket(etcpal_iptype_t             ip_type,
                                          const SacnSendSocketConfig* config,
                                          etcpal_socket_t*            socket)
{
  ...
  if ((res == kEtcPalErrOk) && (config->unicast_sndtimeo_ms > 0))
  {
    res = etcpal_setsockopt(*socket, ETCPAL_SOL_SOCKET, ETCPAL_SO_SNDTIMEO, &config->unicast_sndtimeo_ms,
                            sizeof config->unicast_sndtimeo_ms);

    if (res != kEtcPalErrOk)
    {
      SACN_LOG_ERR("Failed to set SO_SNDTIMEO to %d ms on unicast socket: '%s'", config->unicast_sndtimeo_ms,
                   etcpal_strerror(res));
    }
  }
...
etcpal_error_t create_receive_socket(etcpal_iptype_t       ip_type,
                                     const EtcPalSockAddr* bind_addr,
                                     bool                  set_sockopts,
                                     ReceiveSocket*        socket)
{
 ...
 * [out] ip_addr The multicast address for the given universe and IP type.
 */
void sacn_get_mcast_addr(etcpal_iptype_t ip_type, uint16_t universe, EtcPalIpAddr* ip_addr)
{
 ...
 */
etcpal_error_t sacn_add_receiver_socket(sacn_thread_id_t           thread_id,
                                        etcpal_iptype_t            ip_type,
                                        uint16_t                   universe,
                                        const EtcPalMcastNetintId* netints,
                                        size_t                     num_netints,
 ...
 */
etcpal_error_t queue_subscription(SacnRecvThreadContext*     recv_thread_context,
                                  etcpal_socket_t            sock,
                                  const EtcPalIpAddr*        group,
                                  const EtcPalMcastNetintId* netints,
                                  size_t                     num_netints)
...
/*
 * Poll for network data events on a thread's sockets. Receive & hand off to the process thread.
 *
...
etcpal_error_t sacn_read(SacnRecvThreadContext* recv_thread_context)
{
  ...
  if (poll_res == kEtcPalErrOk)
  {
    ...
    if (poll_event.events & ETCPAL_POLL_IN)
    {
      ...
      // Now that we have the semaphore, we can receive the data.
      uint8_t control_buf[ETCPAL_MAX_CONTROL_SIZE_PKTINFO] = {0};  // Ancillary data

      EtcPalMsgHdr msg = {{0}};
      msg.buf          = recv_thread_context->network_buf;
      msg.buflen       = kSacnMtu;
      ...
      if (recv_res > 0)
      {
        ...
        {
          recv_thread_context->network_read_data.from_addr = msg.name;
          recv_thread_context->network_read_data.data_len  = (size_t)recv_res;
          recv_thread_context->network_read_data.data      = recv_thread_context->network_buf;
          ...
          if (sacn_receiver_sockets_lock())
          {
            ...
            if (index >= 0)
            {
              recv_thread_context->network_read_data.netint.ip_type =
                  recv_thread_context->socket_refs[index].socket.ip_type;
              recv_thread_context->network_read_data.netint.index =
                  recv_thread_context->socket_refs[index].socket.ifindex;
            }
            else
            {
...
/*
 * Poll for read events, filling in SacnReadData if one occurred.
 *
 * Blocks up to SACN_RECEIVER_READ_TIMEOUT_MS waiting for events.
 *
 * [in,out] recv_thread_context Context for the thread calling this function.
 * [out] read_data Filled in with the data if a read event occurred.
 *
 * Returns The event that occurred or kSacnReadEventTimedOut if the timeout elapsed.
 */
sacn_read_event_t sacn_poll(SacnRecvThreadContext* recv_thread_context, SacnReadData* read_data)
{
#if SACN_RECEIVER_ENABLED
  if (!SACN_ASSERT_VERIFY(recv_thread_context) || !SACN_ASSERT_VERIFY(read_data))
    return kSacnReadEventTimedOut;

  sacn_read_event_t read_event = kSacnReadEventTimedOut;
  if (etcpal_queue_timed_receive(&recv_thread_context->read_queue, &read_event, SACN_RECEIVER_READ_TIMEOUT_MS))
  {
    SACN_ASSERT_VERIFY(read_event != kSacnReadEventTimedOut);

    if (read_event == kSacnReadEventNetwork)
      *read_data = recv_thread_context->network_read_data;
    else if (read_event == kSacnReadEventHook)
      *read_data = recv_thread_context->hook_read_data;
  }
...
etcpal_error_t sacn_send_unicast(sacn_ip_support_t   ip_supported,
                                 const uint8_t*      send_buf,
                                 const EtcPalIpAddr* dest_addr,
                                 etcpal_error_t*     last_send_error)
{
...
etcpal_error_t sockets_init(const SacnNetintConfig* netint_config, sacn_networking_type_t net_type)
{
  ...
  // Validation
  if (netint_config && ((netint_config->send_socket_config.unicast_sndtimeo_ms < 0) ||
                        (netint_config->send_socket_config.multicast_sndtimeo_ms < 0)))
  {
  ...
  if ((res == kEtcPalErrOk) && (net_type == kSource))
  {
    ...
    res = init_unicast_send_sockets(&send_socket_config);
  }
...
#if SACN_RECEIVER_ENABLED || DOXYGEN
etcpal_error_t update_sampling_period_netints(SacnInternalNetintArray* receiver_netints,
                                              bool                     currently_sampling,
                                              EtcPalRbTree*            sampling_period_netints,
                                              const SacnNetintConfig*  app_netint_config)
{
...
etcpal_error_t sacn_validate_netint_config(const SacnNetintConfig*   netint_config,
                                           const SacnMcastInterface* sys_netints,
                           ...
                           sacn_networking_type_t  net_type)
{
  ...
  for (size_t i = 0; i < netint_list->num_netints; ++i)
  {
    ...
    if (use_all_netints || app_netint)
    {
      ...
      etcpal_error_t test_result = test_netint(netint, sys_netints, net_type, &send_socket_config);
                           ...
                           const SacnSendSocketConfig* send_socket_config)
{
  ...
  if (net_type == kSource)
  {
    result = test_sacn_source_netint(netint->index, netint->addr.type, &netint->addr, send_socket_config);
  }
  else
  {
...
etcpal_error_t test_sacn_receiver_netint(unsigned int           index,
                                         etcpal_iptype_t        ip_type,
                                         const EtcPalIpAddr*    addr,
                                         SacnSocketsSysNetints* sys_netints)
{
...
etcpal_error_t test_sacn_source_netint(unsigned int                index,
                                       etcpal_iptype_t             ip_type,
                                       const EtcPalIpAddr*         addr,
                                       const SacnSendSocketConfig* config)
{
  if (!SACN_ASSERT_VERIFY(ip_type != kEtcPalIpTypeInvalid) || !SACN_ASSERT_VERIFY(addr) || !SACN_ASSERT_VERIFY(config))
    return kEtcPalErrSys;
  ...
  etcpal_error_t  test_res          = create_multicast_send_socket(&netint_id, config, &new_source_socket);
...
bool add_sacn_sys_netint(const EtcPalMcastNetintId* netint_id,
                         etcpal_error_t             status,
                         SacnSocketsSysNetints*     sys_netints)
{
