...
// 44-byte master key (HARDCODED FOR TESTING): 32B AES key + 12B salt for AES_256_GCM
static const uint8_t kSrtpMasterKey[44] = {0xC1, 0xEE, 0xC3, 0x71, 0x7D, 0xA7, 0x61, 0x95, 0xBB, 0x87, 0x85,
...
typedef void (*SacnUniverseDataInternalCallback)(sacn_receiver_t             receiver_handle,
                                                 const EtcPalSockAddr*       source_addr,
                                                 const SacnRemoteSource*     source_info,
                                                 const SacnRecvUniverseData* universe_data,
                                                 sacn_thread_id_t            thread_id);
typedef void (*SacnSourcesLostInternalCallback)(sacn_receiver_t       handle,
                                                uint16_t              universe,
                                                const SacnLostSource* lost_sources,
                                                size_t                num_lost_sources,
                                                sacn_thread_id_t      thread_id);
typedef void (*SacnSamplingPeriodStartedInternalCallback)(sacn_receiver_t  handle,
                                                          uint16_t         universe,
                                                          sacn_thread_id_t thread_id);
typedef void (*SacnSamplingPeriodEndedInternalCallback)(sacn_receiver_t  handle,
                                                        uint16_t         universe,
                                                        sacn_thread_id_t thread_id);
typedef void (*SacnSourcePapLostInternalCallback)(sacn_receiver_t         handle,
                                                  uint16_t                universe,
                                                  const SacnRemoteSource* source,
                                                  sacn_thread_id_t        thread_id);
typedef void (*SacnSourceLimitExceededInternalCallback)(sacn_receiver_t  handle,
                                                        uint16_t         universe,
                                                        sacn_thread_id_t thread_id);
...
typedef struct SacnRecvThreadContext
{
  ...
  uint8_t           recv_buf[SACN_MTU_ENCRYPTED];
